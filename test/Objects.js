...
class Circle {
    ...
    * @param {Object[]} springs - Lista de Objetos Spring.
    * @param {Object[]} balls   - Lista de Objetos Circle.
    * @param {number} dt_total  - O passo de tempo total (tempo do quadro).
    * @param {number} passos    - Número de sub-passos para dividir o quadro.
    */
    simulation(springs, balls, dt_total, passos = 5) {
        if (passos <= 0) return;
        const dt_sub = dt_total / passos; // sub delta para sub passos
        ...
        for (let i = 0; i < passos; i++) {
            ...
            if (this.canCollide) for (let spring of springs) if (this != spring.node1 && this != spring.node2) this.resolveCollisionSpring(this.collideWithSpring(spring));
        }
        ...
        if (this.canCollide) for (let i = 0; i < 10; i++) for (let ball of balls) {
    ...
    resolveCollisionSpring(collisionData) {
        ...
        // 2. Reposicionamento (Resolve penetração no sub-passo)
        if (this.canMove) {
            this.x += collisionData.normal.x * collisionData.penetration;
            this.y += collisionData.normal.y * collisionData.penetration;
        }

        ///  3. Responde à colisão
        //
        // 3.1. Decompor a velocidade
        const speedAlongNormal = dot(this.vx, this.vy, collisionData.normal.x, collisionData.normal.y);
        const speedAlongTangent = dot(this.vx, this.vy, -collisionData.normal.y, collisionData.normal.x); // speedAlongTangent é o produto escalar com o vetor Tangente (-normal.y, normal.x)

        // O código da imagem verifica se o objeto está se movendo em direção à colisão
        if ((speedAlongNormal >= 0)) return;// Se a velocidade está afastando ou paralela, não há colisão penetrante

        // 3.2. Recompor o vetor de velocidade final
        // Speed = (Componente Normal Invertida * Vetor Normal) * restitution + (Componente Tangente * Vetor Tangente) * (1 - friction)
    ...
    collideWithCircle(other) {
        ...
        const isColliding = dist < radii_sum_sq;
        if (!isColliding) {
            return {
                isColliding: false,
                penetration: 0,
